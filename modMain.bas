...
' Module:        modMain
'
' Description:   These are the common routines you will need to run most
'                applications
'
...
' ***************************************************************************
  ...
  Public Const PGM_NAME              As String = "Password-Passphrase"
  Public Const MAX_SIZE              As Long = 260
  Public Const MAX_WORDS             As Long = 25  ' max word count for phrase
...
'
' ***************************************************************************
  Public gblnIDE_Environment As Boolean
  Public gblnSpecialWithNbrs As Boolean
  Public gastrChars(36)      As String
  Public gstrVersion         As String
  Public gstrPwdFile         As String
  Public gstrCaseIndex       As String
  Public gstrPwdLenIndex     As String
  Public gstrNumberIndex     As String
  Public gstrSpecialIndex    As String
  Public gobjPrng            As cPrng
...
Public Sub Main()
    ...
    If Not AlreadyRunning(App.EXEName) Then
        ...
        If FindRequiredFile("pwd.dat", strPath) Then
         
            InitComctl32  ' manifest file
        
            Erase gastrChars()
            Set gobjPrng = New cPrng
            gstrPwdFile = strPath
            gstrVersion = PGM_NAME & " v" & App.Major & "." & App.Minor & "." & App.Revision
            gblnSpecialWithNbrs = False
            gblnStopProcessing = False          ' preset global stop flag
            gobjPrng.StopProcessing = False
            
            ' Retrieve previous form entries
            ' HKEY_CURRENT_USER\Software\VB and VBA Program Settings\Pwd_n_Phrase
            gstrCaseIndex = GetSetting(PGM_NAME, "Settings", "CaseIndex", "0")         ' Lowercase
            gstrPwdLenIndex = GetSetting(PGM_NAME, "Settings", "PwdLenIndex", "5")     ' 8 chars
            gstrSpecialIndex = GetSetting(PGM_NAME, "Settings", "SpecialIndex", "0")   ' Alphabetic only
            gstrNumberIndex = GetSetting(PGM_NAME, "Settings", "NumberIndex", "0")     ' 1 special char
...
Public Sub TerminateProgram()
    ...
    ' Save form entries
    ' HKEY_CURRENT_USER\Software\VB and VBA Program Settings\Pwd_n_Phrase
    SaveSetting PGM_NAME, "Settings", "CaseIndex", gstrCaseIndex
    SaveSetting PGM_NAME, "Settings", "PwdLenIndex", gstrPwdLenIndex
    SaveSetting PGM_NAME, "Settings", "SpecialIndex", gstrSpecialIndex
    SaveSetting PGM_NAME, "Settings", "NumberIndex", gstrNumberIndex
    
    ' Empty or free from memory all public objects, collections, and arrays
    ' that were used in this application.
    Set gobjPrng = Nothing
    Erase gastrChars()
    
    CloseAllFiles   ' close any open files accessed by this application
    UnloadAllForms  ' Unload any forms and their controls from memory
    ...
    If gblnIDE_Environment Then
        Exit Sub         ' Terminate this application while in the VB IDE
    Else
        ' ExitProcess function ends a process and all its threads
...
Public Sub EmptyCollection(ByRef colData As Collection)
    ...
    Do While colData.Count > 0
        ...
        ' Parse backwards thru collection
        ' and delete data. The backwards
        ' parsing prevents the collection
        ' from having to reindex itself
